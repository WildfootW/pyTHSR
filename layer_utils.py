...
def MinImgCount(tags = ['A', 'B'], task_name = 'apple2orange'):
...
def randReadImg(tag, num, shp=(256, 256, 3), printName=False, task_name='apple2orange'):
    ...
    for _ in range(num):
        ...
        img.load()
        img = img.resize((shp[1], shp[0]), PIL.Image.ANTIALIAS)
        if list(img.size) == ret.shape[1:]:
            ret[_] = np.asarray(img)
        else:
            img_arr = np.asarray(img)
            ret[_] = np.expand_dims(img_arr, axis=2)
